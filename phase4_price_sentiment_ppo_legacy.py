#!/usr/bin/env python3
"""
PharmaTrade-MM Phase 4: PPO Price+Sentiment ablation (legacy/original version).

This is the pre-improvement Phase 4 pipeline:
- no sentiment scaling/clipping
- no robust multi-candidate anti-collapse selection
- warm-start params from phase4/phase3 tuned json
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Phase 4 PPO price+sentiment legacy ablation")
    ...
    p.add_argument("--results-dir", default="results/phase4_price_sentiment_legacy")
...
def main() -> None:
    ...
    vec_train_env = DummyVecEnv([lambda: build_env(args.train_path)])
    model = PPO("MlpPolicy", vec_train_env, verbose=1, seed=args.seed, **ppo_params)
    model.learn(total_timesteps=args.timesteps)
    model.save(str(model_path))
    print("Saved model:", model_path)

    test_env = build_env(args.test_path)
