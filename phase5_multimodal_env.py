#!/usr/bin/env python3
"""
Phase 5 sequence-aware multimodal trading environment with FDA stream.
"""
...
from phase2_trading_env import DEFAULT_TICKERS, RewardConfig, spaces, gym
from phase5_sequence_utils import (
    Phase5SequenceContract,
    load_phase5_sequence_contract,
...
class PharmaSequenceFDAEnv(gym.Env):
    metadata = {"render_modes": ["human"]}

    def __init__(
        self,
        data: pd.DataFrame,
        contract: Phase5SequenceContract,
        ...
        required = (
            ...
            + contract.event_features
        )
        validate_dataframe_columns(self.df, required)
        if self.cfg.use_event_scaling:
            validate_dataframe_columns(self.df, ["is_event_window"])
        ...
        self.observation_space = spaces.Dict(
            {
                ...
                "fda_seq": spaces.Box(
                    low=-np.inf,
                    high=np.inf,
                    shape=(self.window, self.num_assets, len(contract.event_features)),
                    dtype=np.float32,
                ),
    ...
    def _prepare_arrays(self) -> None:
        ...
        for t_idx, date in enumerate(self.dates):
            ...
            fda_arr[t_idx] = day[self.contract.event_features].to_numpy(dtype=np.float32)
    ...
    def _get_observation(self) -> Dict[str, np.ndarray]:
        ...
        fda_seq = self._get_seq_window(self.fda_arr)
    ...
    def step(self, action: np.ndarray):
        ...
        for i in range(self.num_assets):
            ...
            if a == 0:
                ...
                buy_scale = self.cfg.trade_fraction
                if self.cfg.use_event_scaling and is_event[i] == 1:
                    buy_scale *= self.cfg.event_window_scale
...
def make_phase5_sequence_env_from_processed(
    ...
    **env_kwargs,
    ...
    env_cfg = SeqEnvConfig(
        ...
        event_window_scale=float(env_kwargs.pop("event_window_scale", 0.50)),
        use_event_scaling=bool(env_kwargs.pop("use_event_scaling", False)),
        reward_config=env_kwargs.pop("reward_config", RewardConfig()),
    )
    return PharmaSequenceFDAEnv(
        data=df,
        contract=contract,
