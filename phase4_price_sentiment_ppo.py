#!/usr/bin/env python3
"""
PharmaTrade-MM Phase 4: PPO Price+Sentiment ablation (improved script).

Improvements implemented:
- Increased timesteps (default 150k)
- Multi-seed robust config search (mean Sharpe based)
- Sentiment scaling + clipping from train statistics
- Two-stage training: higher exploration early, lower later
- Action-collapse checks (too much hold/sell/buy concentration)
- Smaller-network and lower-LR candidate configs
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Phase 4 PPO price+sentiment ablation")
    ...
    p.add_argument("--results-dir", default="results/phase4_price_sentiment")
...
def build_param_candidates(base: Dict) -> List[Dict]:
    ...
    # Ensure valid batch/n_steps relation.
    for c in cands:
...
def main() -> None:
    ...
    raw_train = pd.read_csv(args.train_path, parse_dates=["date"])
    raw_test = pd.read_csv(args.test_path, parse_dates=["date"])
    scaled_train, scaled_test, scaler_stats = scale_sentiment(raw_train, raw_test, sent_cols, clip=args.sent_clip)

    # Persist scaled datasets for reproducibility and notebook inspection.
    ...
    # Robust config selection (mean Sharpe across multiple seeds on validation subset).
    trial_rows = []
    best_score = -1e18
    best_cfg = None
    best_seed = args.seed
    for cand in candidate_params:
        ...
        # Harder penalties to avoid degenerate policies.
        buy_penalty = 1.0 if mean_buy < 0.05 else 0.0
        hold_penalty = 1.0 if mean_hold > 0.85 else 0.0
        # Require at least weakly non-negative validation Sharpe.
        ...
        if score > best_score:
            best_score = score
            best_cfg = cand
            # pick seed with strongest validation Sharpe for this candidate
    ...
    trials_path = results_dir / "phase4_robust_search_trials.csv"
    ...
    model = train_two_stage(PPO, vec_train_env, selected_cfg, seed=best_seed, total_timesteps=args.timesteps)
    ...
    # Automatic fallback if policy collapses to no-buy behavior.
    if ad_final["buy_ratio"] <= 0.001:
        ...
        model = train_two_stage(PPO, vec_train_env_fb, forced_cfg, seed=123, total_timesteps=args.timesteps)
        model.save(str(model_path))
    ...
    metric_rows = [
        compute_metrics("PPO_PriceSentiment", ppo_equity, ppo_daily, xph_daily),
    ...
    curve_df = pd.DataFrame(
        {
            ...
            "ppo_price_sentiment": ppo_equity.values,
    ...
    action_dist_path = results_dir / "phase4_action_distribution.csv"
    pd.DataFrame([ad_final]).to_csv(action_dist_path, index=False)

    if curve_df.isnull().any().any():
        raise ValueError("NaN found in phase4 equity curve output")
    if not (curve_df[[c for c in curve_df.columns if c != "date"]] > 0).all().all():
        raise ValueError("Non-positive portfolio value found in phase4")
    if len(ppo_actions) != len(ppo_perf):
        raise ValueError("Phase4 action/perf length mismatch")

    # Direct Phase 3 vs Phase 4 comparison table (if phase3 metrics exist)
    p3_path = Path(args.phase3_metrics_path)
    if p3_path.exists():
        p3 = pd.read_csv(p3_path)
        p3_row = p3[p3["strategy"] == "PPO_PriceOnly"].head(1).copy()
        p4_row = metrics_df[metrics_df["strategy"] == "PPO_PriceSentiment"].head(1).copy()
        if not p3_row.empty and not p4_row.empty:
            ...
            # For drawdown, less negative is better -> delta still informative as P4-P3.
