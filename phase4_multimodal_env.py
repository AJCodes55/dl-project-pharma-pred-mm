#!/usr/bin/env python3
"""
Phase 4 sequence-aware multimodal trading environment.
"""
...
from phase4_sequence_utils import SequenceContract, load_sequence_contract, validate_dataframe_columns
...
class PharmaSequenceTradingEnv(gym.Env):
    ...
    def __init__(
        ...
        required = ["date", "ticker", "close"] + contract.price_features + contract.sentiment_features
    ...
    def _get_observation(self) -> Dict[str, np.ndarray]:
        ...
        return {"price_seq": price_seq, "sent_seq": sent_seq, "portfolio_ctx": portfolio_ctx}
...
def make_sequence_env_from_processed(
    dataset_path: str | Path,
    feature_config_path: str | Path,
    window_size: int = 20,
    tickers: Optional[Sequence[str]] = None,
    **env_kwargs,
) -> PharmaSequenceTradingEnv:
    df = pd.read_csv(dataset_path, parse_dates=["date"])
    contract = load_sequence_contract(feature_config_path, window_size=window_size)
    ...
    return PharmaSequenceTradingEnv(
        data=df,
        contract=contract,
